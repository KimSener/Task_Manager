...
// Определение фильтра который выполняется один раз для каждого запроса
    @Override
    ...
    /* Что мы делаем внутри doFilterInternal():
      — получаем JWTиз заголовка Authorization (удалив Bearer префикс)
     ...
     Если нет своей UserDetails то используют схему по умолчанию в которой всего три поля без реализаций UserDetails
